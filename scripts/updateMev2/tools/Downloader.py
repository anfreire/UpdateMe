        ...
        try:
            ...
            with open(path, "wb") as apk:
                for data in r.iter_content(chunk_size=chunk_size):
                    ...
                    print(
                        f"\r[{'=' * done}{' ' * (50 - done)}] {dl / 1000000:.2f}/{total_size / 1000000:.2f}MB",
                        end="",
                    )
            Cursor.showCursor()
  ...
  -H \'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36\''
    ...
    def download_chrome(self, link: str, path: str, timeout=100) -> bool:
        try:
            ...
            os.system(
                f"timeout 150 google-chrome '{link}' 2> /dev/null > /dev/null &"
            )
            checks = timeout / 10 + 1
            new_file = None
            print()
    ...
    @classmethod
    def get_downloaded_file(cls, path: str, fun: callable) -> bool:
        try:
            dowloads = os.listdir("/home/anfreire/Downloads")
            checks = 10
            new_file = None
