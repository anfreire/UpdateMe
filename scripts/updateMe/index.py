...
from constants import GLOBAL, COLORS, PREFIX
...
class IndexManager:
    ...
    def push_git(cls, old_index: Dict[str, IndexInfo]) -> None:
        ...
        if not updated:
            print(
                f"{PREFIX.OK} No need to push to GitHub, no updates"
            )
        ...
        print(f"{PREFIX.OK} Pushed to GitHub")
    ...
    def push_firebase(cls, old_index: Dict[str, IndexInfo] | None = None) -> None:
        ...
        if not updated:
            print(
                f"{PREFIX.OK} No need to push to Firebase, no updates"
            )
            answer = input(f"Do you want to push anyway? [Y/n]")
            if answer.lower() != "y":
                return
        ...
        print(f"{PREFIX.OK} Pushed to Firebase")
    ...
    def send_notifications(cls, old_index: Dict[str, IndexInfo] | None = None) -> None:
        ...
        if len(updates) > 0:
            ...
            print(
                f"{PREFIX.OK} Sent notifications of {len(updates)} updates"
            )
        else:
            print(
                f"{PREFIX.OK} No need to send notifications, no updates"
            )
            return
        ...
        for token in tokens:
            try:
                ...
                print(
                    f"{PREFIX.OK} Sent notification to {token[0:5]}...{token[-5:]}"
                )
            except Exception:
                print(
    ...
    def get_reports() -> None:
        ...
        try:
            ...
            print(
                f"{PREFIX.OK} Got {len(new_reports)} new reports"
            )
        except Exception:
            print(f"{PREFIX.OK} No new reports")
            return
